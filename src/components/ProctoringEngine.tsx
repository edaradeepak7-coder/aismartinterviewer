...
function computeInsights(events: ProctoringEvent[]): ProctoringInsights {
  ...
  if (faceNotDetectedSeconds > 0) summaryParts.push(`presence miss ~${faceNotDetectedSeconds}s`);
...
export default function ProctoringEngine({
  ...
  const lastBlurEventRef = useRef(0);
  const lastPresenceMissRef = useRef(0);
  ...
  // Tab visibility detection (debounced to reduce noise)
  useEffect(() => {
    const handleVisibilityChange = () => {
      if (!document.hidden) return;
      const now = Date.now();
      if (now - lastTabEventRef.current < 5000) return;
    ...
    const handleBlur = () => {
      // Ignore blur while document is hidden (tab_switch already covers that)
      if (document.hidden) return;
      const now = Date.now();
      if (now - lastBlurEventRef.current < 8000) return;
      lastBlurEventRef.current = now;
      addEvent({
        type: 'window_blur',
        timestamp: new Date().toISOString(),
        severity: 'medium',
  ...
  // Camera (presence/lighting) + separate mic stream for audio monitoring
  const startCamera = useCallback(async () => {
    try {
      ...
      // Separate mic stream — video stream has audio:false so this is required
      try {
        ...
        if (AudioContextClass) {
          ...
          audioIntervalRef.current = setInterval(() => {
            ...
            const now = Date.now();
            // Sustained loud ambient / second speaker heuristic
            if (avg > 90 && now - lastSpikeTime > 12000) {
              lastSpikeTime = now;
              addEvent({
                type: 'audio_spike',
                timestamp: new Date().toISOString(),
        ...
        /* mic permission optional for mock interviews */
      }

      // Presence / lighting check (not true face recognition — labeled honestly in UI)
      faceCheckIntervalRef.current = setInterval(() => {
        ...
        for (let i = 0; i < pixels.length; i += 16) {
          ...
          brightnessSum += lum;
          if (prevLum >= 0) contrastSamples += Math.abs(lum - prevLum);
          prevLum = lum;
        }
        const sampleCount = pixels.length / 16;
        const avgBrightness = brightnessSum / sampleCount;
        const avgContrast = contrastSamples / Math.max(1, sampleCount - 1);

        // Covered lens / black frame / washed-out frame → no presence signal
        const present =
          avgBrightness > 18 &&
          avgBrightness < 245 &&
          avgContrast > 4;
        ...
        // Require 2 consecutive misses (~6s) and rate-limit event spam
        if (consecutiveDarkFramesRef.current >= 2) {
          const now = Date.now();
          if (now - lastPresenceMissRef.current > 9000) {
            lastPresenceMissRef.current = now;
            addEvent({
              type: 'face_not_detected',
              timestamp: new Date().toISOString(),
              severity: 'medium',
              detail:
                'Camera presence check failed — frame too dark, covered, or candidate may have moved away',
            });
          }
        }
      }, 3000);
      ...
      <div className="flex items-center gap-3 px-3 py-2 bg-[#0F1923] border border-[#1E2D3D] rounded-xl">
        ...
        <div className="flex items-center gap-1.5">
          ...
          <span className={`text-[10px] font-600 ${faceDetected ? 'text-emerald-400' : 'text-red-400'}`}>
            {faceDetected ? 'Presence OK' : 'No Presence'}
          </span>
        </div>
        <div className="w-px h-3 bg-[#1E2D3D]" />
        <div className="flex items-center gap-1.5">
          <Camera size={11} className={videoStreamRef.current ? 'text-emerald-400' : 'text-[#4A6B7A]'} />
          <span className="text-[10px] text-[#4A6B7A]">Cam</span>
        </div>
        <div className="w-px h-3 bg-[#1E2D3D]" />
        <div className="flex items-center gap-1.5">
          <Mic size={11} className={audioStreamRef.current ? 'text-emerald-400' : 'text-[#4A6B7A]'} />
