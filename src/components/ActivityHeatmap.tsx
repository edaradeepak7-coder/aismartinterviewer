  ...
  for (let w = 11; w >= 0; w--) {
    for (let d = 0; d < 7; d++) {
      ...
      cells.push({
        date: key,
        value: 0,
        label: `${date.toLocaleDateString('en-US', { month: 'short', day: 'numeric' })}: 0 sessions`,
      });
    }
  }
  return cells;
  ...
  for (let d = 0; d < 7; d++) {
    for (let h = 0; h < 24; h++) {
      cells.push({
        date: `${days[d]}-${h}`,
        value: 0,
        label: `${days[d]} ${h}:00 — 0 interviews`,
      });
    }
  }
  return cells;
...
export default function ActivityHeatmap({
  ...
  const fallback = useMemo(
    () => (mode === 'hourly' ? getEmptyHourlyCells() : getEmptyWeeklyCells()),
    [mode]
  );

  const [cells, setCells] = useState<HeatmapCell[]>(() => data ?? fallback);

  useEffect(() => {
    setCells(data && data.length > 0 ? data : fallback);
  }, [data, fallback]);

  const colors = COLOR_SCHEMES[colorScheme];
  const legendColors = LEGEND_COLORS[colorScheme];
  const hasActivity = cells.some((c) => c.value > 0);
      ...
      <div className="bg-white rounded-2xl border border-[#E8ECF4] shadow-sm p-5" data-tour={dataTour}>
        <div className="flex items-start justify-between mb-4">
          <div>
            ...
            {!hasActivity && (
              <p className="text-xs text-[#6B7A99] mt-1">No interview activity in this window yet.</p>
            )}
          </div>
