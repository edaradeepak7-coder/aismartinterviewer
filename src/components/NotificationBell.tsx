...
import { Bell, CheckCheck, X, Award, Gift, CalendarCheck, BarChart2, Info, Loader2, ClipboardList, Star, Clock, Inbox, Wifi, CreditCard } from 'lucide-react';
...
const typeConfig: Record<NotifType, { icon: React.ReactNode; dot: string; iconBg: string }> = {
  ...
  interview_reminder: { icon: <Clock size={14} />, dot: 'bg-amber-400', iconBg: 'bg-amber-400/15 text-amber-400' },
  ...
  seat_purchase: { icon: <CreditCard size={14} />, dot: 'bg-emerald-500', iconBg: 'bg-emerald-500/15 text-emerald-500' },
};

const deepLinkMap: Partial<Record<NotifType, string>> = {
  interview_scheduled: '/invitations',
  interview_reminder: '/invitations',
  assessment_assigned: '/assessments',
  shortlisted: '/jobs',
  offer_accepted: '/job-offers',
  offer_declined: '/job-offers',
  offer_received: '/job-offers',
  task_due: '/crm',
  score_ready: '/interview-results',
  booking_confirmed: '/invitations',
  interview_completed: '/interview-results',
  seat_purchase: '/institution-subscription',
};
...
/** Demo-only preview when not signed in and DEMO_MODE is on */
const PREVIEW_MOCKS: DBNotification[] = [
  {
    ...
    is_read: false, action_url: '/invitations', metadata: {},
    created_at: new Date(Date.now() - 1000 * 60 * 15).toISOString(),
  },
  {
    id: 'pm-2', user_id: 'demo', type: 'offer_received',
    title: 'Offer Received',
    message: 'You have a pending offer for Full Stack Developer.',
    is_read: false, action_url: '/job-offers', metadata: {},
