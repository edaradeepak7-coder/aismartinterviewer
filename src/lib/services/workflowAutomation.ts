...
function mapNotificationType(type: string): string {
  if (NOTIFICATION_TYPES.has(type)) return type;
  const aliases: Record<string, string> = {
    enrollment: 'system',
    course_update: 'system',
    assessment_complete: 'system',
    interview_unlock: 'interview_scheduled',
    achievement: 'system',
    leaderboard: 'system',
  };
  return aliases[type] || 'system';
}
...
async function dispatchNotification(params: {
  ...
  const type = mapNotificationType(params.type);
  try {
    if (typeof window === 'undefined') {
      ...
      if (error) {
        console.warn('[WorkflowAutomation] Notification insert failed:', error.message);
      }
      return;
    }
    ...
    console.warn('[WorkflowAutomation] Notification dispatch error:', err);
  }
}
