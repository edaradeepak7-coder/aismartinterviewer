...
export default function InboxContent() {
  ...
  const loadMessages = useCallback(async () => {
    ...
    if (error || !data || data.length === 0) {
      ...
      // Map notifications to inbox format
      const mapped: InboxMessage[] = data.map((n: any) => ({
        id: n.id,
        user_id: n.user_id,
  ...
  function mapTypeToCategory(type: string): InboxCategory {
    if (['interview_scheduled', 'booking_confirmed', 'interview_completed'].includes(type)) return 'interview_reminder';
  ...
  function getActionLabel(type: string): string {
    const map: Record<string, string> = {
      interview_scheduled: 'View Interview Details',
      booking_confirmed: 'Open Calendar',
      ...
      <div className="max-w-3xl mx-auto space-y-5 fade-in">
        ...
        {useMock && (
          <p className="text-center text-[11px] text-muted-foreground/50">
            Showing demo data — connect to Supabase to see live notifications
          </p>
        )}
      </div>
    </AppLayout>
