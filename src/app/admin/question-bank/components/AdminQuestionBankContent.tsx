...
const TABLE_CONFIG: Record<TableKey, { label: string; color: string; bg: string; border: string; icon: React.ReactNode; accent: string }> = {
  technical: {
...
export default function AdminQuestionBankContent() {
  ...
  // Modal state
  const [modal, setModal] = useState<{ mode: 'create' | 'edit' | 'preview'; record?: AirtableRecord } | null>(null);
  ...
  const fetchTable = useCallback(async (tableKey: TableKey) => {
    ...
    try {
      const res = await fetch(`/api/airtable/question-bank?table=${tableKey}`);
      const data = await res.json();
      if (!res.ok) throw new Error(data.error || 'Failed to fetch');
      setTables(prev => ({ ...prev, [tableKey]: { records: data.records || [], loading: false, error: null } }));
    } catch (err: unknown) {
  ...
  const openEdit = (record: AirtableRecord) => {
    setForm({
      Question: record.fields.Question || '',
      Category: record.fields.Category || '',
      Difficulty: record.fields.Difficulty || 'Medium',
      'Job Role': record.fields['Job Role'] || '',
  ...
  const openPreview = (record: AirtableRecord) => {
  ...
  const handleSave = async () => {
    ...
    try {
      if (modal?.mode === 'create') {
        const res = await fetch('/api/airtable/question-bank', {
          method: 'POST',
          headers: { 'Content-Type': 'application/json' },
          body: JSON.stringify({ tableKey: activeTab, fields: form }),
        });
        const data = await res.json();
        ...
        setTables(prev => ({
          ...
          [activeTab]: {
            ...prev[activeTab],
            records: [data.record, ...prev[activeTab].records],
          },
        }));
      } else if (modal?.mode === 'edit' && modal.record) {
        const res = await fetch(`/api/airtable/question-bank/${modal.record.id}`, {
          method: 'PATCH',
          headers: { 'Content-Type': 'application/json' },
          body: JSON.stringify({ tableKey: activeTab, fields: form }),
        });
        ...
        setTables(prev => ({
          ...
          [activeTab]: {
            ...
            records: prev[activeTab].records.map(r =>
              r.id === modal.record!.id ? { ...r, fields: { ...r.fields, ...form } } : r
            ),
          },
        }));
      }
  ...
  const handleDelete = async () => {
    ...
    setDeleting(true);
    try {
      const res = await fetch(`/api/airtable/question-bank/${deleteConfirm.id}?tableKey=${deleteConfirm.tableKey}`, {
        method: 'DELETE',
      });
      if (!res.ok) {
        const data = await res.json();
  ...
  const filteredRecords = currentTable.records.filter(r => {
    const q = r.fields.Question?.toLowerCase() || '';
    const cat = r.fields.Category?.toLowerCase() || '';
    const role = r.fields['Job Role']?.toLowerCase() || '';
    const matchSearch = !search || q.includes(search.toLowerCase()) || cat.includes(search.toLowerCase()) || role.includes(search.toLowerCase());
    const matchDiff = difficultyFilter === 'All' || r.fields.Difficulty === difficultyFilter;
    const matchStatus = statusFilter === 'All' || r.fields.Status === statusFilter;
      ...
      <div className="min-h-screen bg-slate-50">
            ...
            <div className="flex items-center gap-3">
              ...
              <div>
                ...
                <p className="text-xs text-slate-500 mt-0.5">
                  Synced with Airtable · {totalAll} total questions
                  {lastSynced && (
        ...
        <div className="px-6 py-4 grid grid-cols-3 gap-4">
          {(Object.keys(TABLE_CONFIG) as TableKey[]).map(key => {
            ...
            const active = tables[key].records.filter(r => r.fields.Status === 'Active').length;
            return (
              <button
                key={key}
          ...
          <div className="bg-white rounded-xl border border-slate-200 overflow-hidden">
            ...
            {currentTable.loading ? (
              <div className="flex items-center justify-center py-16 gap-2 text-slate-400">
                ...
                <span className="text-sm">Loading from Airtable…</span>
              </div>
            ) : currentTable.error ? (
                ...
                <table className="w-full text-sm">
                  <thead>
                    <tr className="bg-slate-50 border-b border-slate-100">
                      ...
                      <th className="text-left px-4 py-2.5 text-xs font-medium text-slate-500">Job Role</th>
                      ...
                      <tr key={record.id} className="hover:bg-slate-50/60 transition-colors group">
                        ...
                        <td className="px-4 py-3">
                          <p className="text-slate-800 font-medium line-clamp-2 leading-snug">
                            {record.fields.Question || <span className="text-slate-400 italic">No question text</span>}
                          </p>
                        </td>
                        <td className="px-4 py-3">
                          <span className="text-slate-600 text-xs">{record.fields.Category || '—'}</span>
                        </td>
                        <td className="px-4 py-3">
                          {record.fields.Difficulty ? (
                            <span className={`text-xs px-2 py-0.5 rounded-full font-medium ${DIFFICULTY_STYLES[record.fields.Difficulty] || 'bg-slate-100 text-slate-600'}`}>
                              {record.fields.Difficulty}
                            </span>
                          ) : <span className="text-slate-400">—</span>}
                        </td>
                        <td className="px-4 py-3">
                          <span className="text-slate-600 text-xs">{record.fields['Job Role'] || '—'}</span>
                        </td>
                        <td className="px-4 py-3">
                          {record.fields.Status ? (
                            <span className={`text-xs px-2 py-0.5 rounded-full font-medium ${STATUS_STYLES[record.fields.Status] || 'bg-slate-100 text-slate-600'}`}>
                              {record.fields.Status}
                            </span>
                          ) : <span className="text-slate-400">—</span>}
                        </td>
          ...
          <div className="bg-white rounded-2xl shadow-2xl w-full max-w-lg">
            ...
            <div className="px-6 py-5 space-y-4">
              ...
              <div className="grid grid-cols-2 gap-4">
                ...
                <div>
                  <label className="block text-xs font-medium text-slate-700 mb-1.5">Job Role</label>
                  <input
                    type="text"
                    value={form['Job Role']}
            ...
            <div className="px-6 py-5 space-y-4">
              <div className={`p-4 rounded-xl ${config.bg} ${config.border} border`}>
                <p className="text-sm font-medium text-slate-800 leading-relaxed">
                  {modal.record.fields.Question || 'No question text'}
                </p>
              </div>
              <div className="grid grid-cols-2 gap-3">
                {[
                  { label: 'Table', value: config.label },
                  { label: 'Category', value: modal.record.fields.Category },
                  { label: 'Difficulty', value: modal.record.fields.Difficulty },
                  { label: 'Job Role', value: modal.record.fields['Job Role'] },
                  { label: 'Status', value: modal.record.fields.Status },
                  { label: 'Record ID', value: modal.record.id },
                ].map(({ label, value }) => (
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/40 backdrop-blur-sm p-4">
          <div className="bg-white rounded-2xl shadow-2xl w-full max-w-sm p-6">
            ...
            <p className="text-sm text-slate-600 mb-5">
              Are you sure you want to permanently delete this question from Airtable?
            </p>
