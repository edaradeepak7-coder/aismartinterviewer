...
import {
  Building2, Clock, CheckCircle, XCircle, Eye, Search, Download, AlertCircle,
  CreditCard, Layers, X, Loader2, Phone, Globe, MapPin, User, Mail, Check, Ban, RefreshCw,
} from 'lucide-react';
...
function ReviewModal({
  ...
  onClose: () => void;
  onSuccess: (updated: InstitutionApplication) => void;
}) {
  ...
  const handleSubmit = async () => {
    ...
    try {
      const body =
        action === 'approve'
          ? { action: 'approve' as const, seats_allocated: seatsAllocated }
          : { action: 'reject' as const, rejection_reason: rejectionReason.trim() };

      const res = await fetch(`/api/admin/institution-applications/${app.id}`, {
        method: 'PATCH',
      ...
      const updated = (json.data || json) as InstitutionApplication;
...
export default function InstitutionApplicationsContent() {
  ...
  const loadApplications = useCallback(async () => {
    ...
    setError(null);
    try {
      const res = await fetch('/api/admin/institution-applications');
      const json = await res.json().catch(() => ({}));
      if (!res.ok) {
        throw new Error(json.error || 'Failed to load applications');
      }
      setApplications((json.data || []) as InstitutionApplication[]);
    } catch (err: unknown) {
  ...
  const handleReviewSuccess = (updated: InstitutionApplication) => {
  ...
  const exportCsv = () => {
    const headers = [
      'Institution', 'Type', 'Contact', 'Email', 'Phone', 'Seats Requested', 'Seats Allocated',
      'Payment Method', 'Amount', 'Reference', 'Status', 'Applied', 'Rejection Reason',
    ];
    const rows = filtered.map(a => [
      escapeCsv(a.institution_name),
      escapeCsv(a.institution_type),
      escapeCsv(a.contact_name),
      escapeCsv(a.contact_email),
      escapeCsv(a.contact_phone),
      escapeCsv(a.seats_requested),
      escapeCsv(a.seats_allocated),
      escapeCsv(a.payment_method),
      escapeCsv(a.payment_amount),
      escapeCsv(a.payment_reference),
      escapeCsv(a.status),
      escapeCsv(a.created_at ? new Date(a.created_at).toLocaleDateString() : ''),
      escapeCsv(a.rejection_reason),
    ].join(','));
    ...
    link.href = url;
    link.download = `institution-applications-${new Date().toISOString().slice(0, 10)}.csv`;
    link.click();
    ...
    <AppLayout role="admin">
      <div className="space-y-6 fade-in">
        ...
        <div className="flex items-start justify-between gap-4">
          ...
          <div className="flex items-center gap-2 shrink-0">
            <button
              ...
              disabled={loading}
              className="flex items-center gap-1.5 px-3 py-2 text-sm font-500 text-[#6B7A99] border border-[#E8ECF4] rounded-lg hover:bg-[#F4F6FA] bg-white disabled:opacity-50"
            >
            ...
            <button
              ...
              className="hidden sm:flex items-center gap-1.5 px-3 py-2 text-sm font-500 text-[#6B7A99] border border-[#E8ECF4] rounded-lg hover:bg-[#F4F6FA] bg-white disabled:opacity-50"
            >
        ...
        <div className="flex flex-wrap gap-3">
          ...
          <select
            value={paymentFilter}
            onChange={e => setPaymentFilter(e.target.value as 'all' | 'online' | 'offline')}
            className="px-3 py-2 text-sm border border-[#E8ECF4] rounded-lg focus:outline-none focus:ring-2 focus:ring-teal-500/20 bg-white text-[#0D1B3E]"
          >
        ...
        <div className="bg-white border border-[#E8ECF4] rounded-xl overflow-hidden">
          {loading ? (
            <div className="flex flex-col items-center justify-center py-16 gap-3 text-[#6B7A99]">
            ...
            <div className="flex flex-col items-center justify-center py-16 gap-3 px-4">
              ...
              <button
                onClick={loadApplications}
                className="flex items-center gap-1.5 px-4 py-2 text-sm font-600 bg-teal-600 text-white rounded-lg hover:bg-teal-700"
              >
            ...
            <div className="overflow-x-auto">
              <table className="w-full text-sm min-w-[900px]">
                ...
                <tbody className="divide-y divide-[#F4F6FA]">
                  {filtered.map((app, idx) => (
                    <tr key={app.id} className="hover:bg-[#F8FAFC] transition-colors">
                      ...
                      <td className="px-4 py-3">
                        ...
                        {app.seats_allocated != null && app.seats_allocated !== app.seats_requested && (
                      ...
                      <td className="px-4 py-3 font-700 text-[#0D1B3E]">
                        {app.payment_amount != null ? `₹${app.payment_amount.toLocaleString()}` : '—'}
                      </td>
                  ...
                  {filtered.length === 0 && (
                    <tr>
                      <td colSpan={9} className="px-4 py-12 text-center text-sm text-[#6B7A99]">
