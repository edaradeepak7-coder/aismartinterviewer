...
import { Calendar, Clock, ChevronLeft, ChevronRight, CheckCircle, XCircle, Loader2, Globe, User, Plus, Trash2, RefreshCw, CalendarDays, Shield, Info, ChevronDown } from 'lucide-react';
...
function formatTimeInTz(timeStr: string, dateStr: string, fromTz: string, toTz: string): string {
  try {
...
function SlotCard({ slot, tz, onConfirm, onCancel, onBook, isRecruiter, loading }: {
  ...
  const startFormatted = formatTimeInTz(slot.start_time, slot.slot_date, 'UTC', tz);
  const endFormatted = formatTimeInTz(slot.end_time, slot.slot_date, 'UTC', tz);
...
export default function InterviewCalendarContent() {
  ...
  useEffect(() => {
    ...
    supabase.from('user_profiles').select('role').eq('id', user.id).maybeSingle()
      .then(({ data }) => {
        if (data?.role === 'recruiter' || data?.role === 'admin') setUserRole('recruiter');
  ...
  const handleBook = async (slotId: string) => {
    ...
    try {
      ...
      await supabase.from('recruiter_availability').update({ status: 'booked' }).eq('id', slotId);
      showToast('Slot booked! Awaiting confirmation.', 'success');
      fetchSlots();
    } catch (err: any) {
  ...
  const handleConfirm = async (slotId: string) => {
    ...
    try {
      ...
      await supabase.from('interview_bookings').update({ confirmed: true }).eq('id', slot.booking.id);
  ...
  const handleCancel = async (slotId: string) => {
    setActionLoading(true);
    try {
      await supabase.from('recruiter_availability').update({ status: 'cancelled' }).eq('id', slotId);
  ...
  const handleAddSlot = async () => {
    ...
    try {
      ...
      const { error } = await supabase.from('recruiter_availability').insert({
  ...
  useEffect(() => {
    ...
    const conflict = isConflict(slots, newSlot.slot_date, newSlot.start_time, newSlot.duration);
    ...
    <div className="max-w-6xl mx-auto px-4 sm:px-6 py-8 space-y-6">
      ...
      <div className="flex items-start justify-between gap-4 flex-wrap">
        <div>
          ...
          <p className="text-sm text-muted-foreground mt-1">
            {userRole === 'recruiter' ? 'Manage your availability and confirm bookings' : 'Browse and book available interview slots'}
          </p>
        </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-5 gap-6">
        ...
        <div className="lg:col-span-3 space-y-4">
          ...
          {/* Add slot form */}
          {showAddForm && userRole === 'recruiter' && (
          ...
          {loading ? (
            ...
            <div className="flex flex-col items-center justify-center py-16 text-center bg-card border border-border rounded-xl">
              ...
              <p className="text-xs text-muted-foreground mt-1">
                {userRole === 'recruiter' ? 'Click "Add Slot" to create availability.' : 'Select another date to find open slots.'}
              </p>
            </div>
          ) : (
