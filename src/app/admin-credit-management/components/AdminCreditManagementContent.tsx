...
export default function AdminCreditManagementContent() {
  ...
  const loadBatches = useCallback(async () => {
    ...
    setLoadError(null);
    try {
      const res = await fetch('/api/admin/credit-adjustments');
  ...
  const loadBatchLines = async (batchId: string) => {
    ...
    try {
      const res = await fetch(`/api/admin/credit-adjustments?batch_id=${batchId}`);
      const json = await res.json();
      setBatchLines(prev => ({ ...prev, [batchId]: (json.data as AdjustmentLine[]) || [] }));
    } catch {
  ...
  // ─── Render ───────────────────────────────────────────────────────────────
  return (
    <div className="bg-[#0A0F1E] text-white p-4 md:p-6">
      {/* Header */}
      <div className="mb-6">
