...
export default function OfferStatusContent() {
  ...
  return (
    <div className="max-w-2xl mx-auto px-4 sm:px-6 py-8 space-y-6">
      ...
      {isAccepted && offer.next_steps && offer.next_steps.length > 0 && (
        <div className="bg-card border border-border rounded-xl p-5 space-y-3">
          <h2 className="text-sm font-600 text-foreground">Next Steps</h2>
          <div className="space-y-2">
            {offer.next_steps.map((step: any, idx: number) => (
      ...
      {isAccepted && offer.interview_prep_tips && offer.interview_prep_tips.length > 0 && (
        <div className="bg-card border border-border rounded-xl p-5 space-y-3">
          <div className="flex items-center gap-2">
            <BookOpen size={16} className="text-primary" />
            <h2 className="text-sm font-600 text-foreground">Interview Prep Tips</h2>
          </div>
          <div className="space-y-2">
            {offer.interview_prep_tips.map((tip: any, idx: number) => (
      ...
      <div className="flex items-center gap-3 flex-wrap">
        ...
        {isAccepted && (
          ...
          >
            Book Interview Slot <ArrowRight size={14} />
          </Link>
        )}
        {isPending && (
          <button
