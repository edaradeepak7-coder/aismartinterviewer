...
import { Shield, Users, Building2, Briefcase, GraduationCap, UserCheck, ClipboardList, Cpu, Save, Check, ChevronDown, ChevronRight, Search, Globe, Lock, Eye, Download, Settings, Info, Loader2, RefreshCw } from 'lucide-react';
...
const ROLES_BASE: Omit<Role, 'userCount'>[] = [
  { id: 'super_admin', name: 'Super Admin', description: 'Full platform-wide access and control', icon: <Shield size={18} />, color: 'bg-red-50 text-red-600 border-red-200', isSystem: true },
  { id: 'institution_admin', name: 'Institution Admin', description: 'Manages an educational institution', icon: <GraduationCap size={18} />, color: 'bg-blue-50 text-blue-600 border-blue-200', isSystem: true },
  { id: 'org_admin', name: 'Organization Admin', description: "Manages an organization's recruitment", icon: <Building2 size={18} />, color: 'bg-violet-50 text-violet-600 border-violet-200', isSystem: true },
  { id: 'recruiter', name: 'Recruiter', description: 'Conducts recruitment and evaluates candidates', icon: <Briefcase size={18} />, color: 'bg-teal-50 text-teal-600 border-teal-200', isSystem: true },
  { id: 'placement_officer', name: 'Placement Officer', description: 'Manages institutional placements and drives', icon: <UserCheck size={18} />, color: 'bg-amber-50 text-amber-600 border-amber-200', isSystem: true },
  { id: 'evaluator', name: 'Evaluator', description: 'Conducts or reviews assessments and interviews', icon: <ClipboardList size={18} />, color: 'bg-orange-50 text-orange-600 border-orange-200', isSystem: true },
  { id: 'faculty', name: 'Faculty', description: 'Manages students, learning and placement prep', icon: <Cpu size={18} />, color: 'bg-indigo-50 text-indigo-600 border-indigo-200', isSystem: true },
  { id: 'candidate', name: 'Candidate', description: 'Takes assessments, interviews and applies for jobs', icon: <Users size={18} />, color: 'bg-green-50 text-green-600 border-green-200', isSystem: true },
];
...
export default function RBACContent() {
  ...
  const ROLES: Role[] = ROLES_BASE.map(r => ({
    ...r,
  ...
  const load = useCallback(async () => {
    ...
    try {
      ...
      if (!res.ok) throw new Error(json.error || 'Failed to load RBAC data');
      setUsers(json.data || []);
      setRoleCounts(json.roleCounts || {});
      setTenants(
        (json.tenants || []).map((t: Tenant & { assignedRoles?: string[] }) => ({
          ...t,
          assignedRoles: (t.assignedRoles || []).filter((r): r is RoleId =>
            ROLES_BASE.some(rb => rb.id === r),
          ) as RoleId[],
        })),
      );
    } catch (e: unknown) {
  ...
  const TABS_CONFIG: { id: Tab; label: string; icon: React.ReactNode }[] = [
    ...
    { id: 'tenants', label: 'Tenants & Users', icon: <Globe size={15} /> },
  ];
    ...
    <div className="space-y-6">
      <div className="flex items-start justify-between gap-4">
        <div className="flex items-center gap-3">
          ...
          <div>
            ...
            <p className="text-sm text-[#6B7A99]">Live roles from user_profiles — permission matrix is reference (not DB-backed)</p>
          </div>
        </div>
        <button onClick={load} className="flex items-center gap-2 px-4 py-2 border border-[#DDE3EE] rounded-lg text-sm font-600 text-[#6B7A99] hover:text-[#0D1B3E]">
      ...
      {loading && (
        <div className="flex items-center justify-center py-16 text-[#6B7A99] gap-2 text-sm">
          <Loader2 size={18} className="animate-spin" /> Loading RBAC data…
        </div>
      )}
            ...
            <div className="px-5 py-4 border-b border-[#DDE3EE] flex items-center justify-between gap-4 flex-wrap">
              <div className="flex items-center gap-3">
                ...
                <div>
                  ...
                  <p className="text-xs text-[#6B7A99]">{rolePerms.length} reference permissions · {selectedRoleData.userCount} live users</p>
                </div>
              </div>
              <div className="flex items-center gap-2">
                ...
                <button
                  type="button"
                  disabled
                  title="Role permissions are reference-only; assign roles on users below"
                  className="flex items-center gap-1.5 px-3 py-1.5 bg-[#E8EDF5] text-[#6B7A99] text-xs font-600 rounded-lg cursor-not-allowed"
                >
                  <Save size={12} /> Save Changes
                </button>
              </div>
            </div>
            <p className="text-[11px] text-[#6B7A99] px-4 pt-2">
              Role permissions are reference-only; assign roles on users below.
            </p>
      ...
      {!loading && activeTab === 'tenants' && (
        <div className="space-y-6">
          <div>
            <h3 className="text-sm font-700 text-[#0D1B3E] mb-3">Users ({users.length})</h3>
            {users.length === 0 ? (
              <div className="text-center py-10 text-sm text-[#6B7A99] border border-dashed border-[#DDE3EE] rounded-xl">No users found</div>
            ) : (
              <div className="bg-white border border-[#DDE3EE] rounded-xl overflow-hidden">
                  ...
                  <table className="w-full">
                    <thead className="sticky top-0 bg-[#F9FAFB]">
                      <tr className="border-b border-[#DDE3EE]">
                        <th className="text-left px-4 py-3 text-xs font-700 text-[#6B7A99] uppercase">User</th>
                        <th className="text-left px-4 py-3 text-xs font-700 text-[#6B7A99] uppercase">Role</th>
                        <th className="text-left px-4 py-3 text-xs font-700 text-[#6B7A99] uppercase">Change</th>
                      </tr>
                    </thead>
                      ...
                      {users.slice(0, 100).map(u => (
                        <tr key={u.id} className="hover:bg-[#FAFBFC]">
                          <td className="px-4 py-3">
                            <div className="text-sm font-600 text-[#0D1B3E]">{u.full_name || '—'}</div>
                            <div className="text-xs text-[#6B7A99]">{u.email}</div>
                          </td>
                          <td className="px-4 py-3 text-xs font-mono text-[#3D5A80]">{u.role}</td>
                          <td className="px-4 py-3">
                            <select
                              value={u.role}
                              disabled={updatingUserId === u.id}
                              onChange={e => changeUserRole(u.id, e.target.value)}
                              className="text-xs border border-[#DDE3EE] rounded-lg px-2 py-1.5 bg-white"
                            >
                              {ROLES_BASE.map(r => (
          ...
          <div className="space-y-4">
            <div className="flex items-center justify-between gap-4">
              <h3 className="text-sm font-700 text-[#0D1B3E]">Tenants ({tenants.length})</h3>
              <div className="relative">
                <Search size={14} className="absolute left-3 top-1/2 -translate-y-1/2 text-[#6B7A99]" />
                <input
                  type="text"
            ...
            {filteredTenants.length === 0 ? (
              <div className="text-center py-10 text-sm text-[#6B7A99] border border-dashed border-[#DDE3EE] rounded-xl">
                No institutions/tenants found
              </div>
            ) : (
              <div className="bg-white border border-[#DDE3EE] rounded-xl overflow-hidden">
                  ...
                  <table className="w-full">
                    <thead>
                      <tr className="border-b border-[#DDE3EE] bg-[#F9FAFB]">
                        ...
                        <th className="text-left px-4 py-3 text-xs font-700 text-[#6B7A99] uppercase tracking-wider">Roles Present</th>
                    ...
                    <tbody className="divide-y divide-[#F4F6FA]">
                      {filteredTenants.map(tenant => (
                        <tr key={tenant.id} className="hover:bg-[#FAFBFC] transition-colors">
                          ...
                          <td className="px-4 py-4 text-sm text-[#3D5A80]">{tenant.plan}</td>
                          <td className="px-4 py-4 text-sm text-[#3D5A80] font-500">{tenant.users.toLocaleString()}</td>
                          <td className="px-4 py-4">
                            <div className="flex flex-wrap gap-1.5">
                              {tenant.assignedRoles.length === 0 ? (
                                <span className="text-xs text-[#6B7A99]">—</span>
                              ) : (
                                tenant.assignedRoles.map(roleId => (
                                  <span key={roleId} className="text-[10px] font-600 px-2 py-1 rounded-full border bg-[#0D9488]/10 text-[#0D9488] border-[#0D9488]/30">
                                    {roleId}
                                  </span>
                                ))
                              )}
