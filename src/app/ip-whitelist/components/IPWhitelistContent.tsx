  ...
  const fetchData = useCallback(async () => {
    ...
    try {
      const [ipRes, evRes] = await Promise.all([
        ...
        fetch('/api/admin/ip-whitelist/events'),
      ]);

      if (!ipRes.ok) {
        const err = await ipRes.json().catch(() => ({}));
        throw new Error(err.error || `Failed to load IP whitelist (${ipRes.status})`);
      }
      const ipJson = await ipRes.json();
      setEntries(Array.isArray(ipJson.data) ? ipJson.data : []);

      if (evRes.ok) {
        const evJson = await evRes.json();
        setEvents(Array.isArray(evJson.data) ? evJson.data : []);
      } else {
        setEvents([]);
      }
    } catch (err: any) {
      setEntries([]);
      setEvents([]);
      setLoadError(err?.message || 'Failed to load IP whitelist');
    } finally {
      setLoading(false);
    }
  }, []);
  ...
  const handleAdd = async () => {
    ...
    try {
      ...
      if (!res.ok) {
        ...
        showToast(err.error || 'Failed to add IP', 'error');
        return;
      }
      ...
      <div className="space-y-6 p-6">
        ...
        {loadError && (
          <div className="flex items-start gap-2 rounded-lg border border-amber-500/30 bg-amber-500/10 px-4 py-3">
            ...
            <div>
              <p className="text-sm font-600 text-amber-300">Could not load whitelist</p>
