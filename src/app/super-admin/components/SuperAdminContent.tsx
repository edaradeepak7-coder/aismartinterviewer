...
function OverviewTab() {
  const [activeGrowthView, setActiveGrowthView] = useState<'acquisition' | 'velocity'>('acquisition');
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-2 sm:grid-cols-4 gap-3">
        {platformStats.map(s => (
          <div key={s.label} className="bg-white rounded-2xl border border-[#E8ECF4] p-4 hover:shadow-md transition-shadow">
            <div className="flex items-start justify-between mb-3">
              <div className={`w-9 h-9 rounded-xl ${s.color} flex items-center justify-center`}>{s.icon}</div>
              <span className={`text-xs font-600 flex items-center gap-0.5 ${s.up ? 'text-green-600' : 'text-red-500'}`}>
      ...
      <div>
        <h3 className="text-sm font-700 text-[#0D1B3E] mb-3">Engagement Metrics</h3>
        <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-6 gap-3">
          {engagementMetrics.map(m => (
            <div key={m.label} className="bg-white rounded-xl border border-[#E8ECF4] p-3">
              <div className={`inline-flex items-center px-2 py-0.5 rounded-full text-[10px] font-600 mb-2 ${m.color}`}>
                {m.up ? <ArrowUpRight size={10} className="mr-0.5" /> : <ArrowDownRight size={10} className="mr-0.5" />}{m.change}
              </div>
              <p className="text-lg font-800 text-[#0D1B3E]">{m.value}</p>
              <p className="text-[11px] font-600 text-[#0D1B3E] leading-tight">{m.label}</p>
              <p className="text-[10px] text-[#9BA8C0] mt-0.5">{m.sub}</p>
            </div>
          ))}
        </div>
      </div>
      ...
      <div className="bg-white rounded-2xl border border-[#E8ECF4] p-5">
        <div className="flex items-center justify-between mb-4 flex-wrap gap-2">
          <div>
            <h3 className="text-sm font-700 text-[#0D1B3E]">Growth Charts</h3>
            <p className="text-xs text-[#6B7A99]">User acquisition & content publishing velocity</p>
          </div>
          <div className="flex gap-1 bg-[#F4F6FA] rounded-lg p-1">
            <button onClick={() => setActiveGrowthView('acquisition')} className={`px-3 py-1.5 text-xs font-600 rounded-md transition-all ${activeGrowthView === 'acquisition' ? 'bg-white text-[#0D9488] shadow-sm' : 'text-[#6B7A99]'}`}>User Acquisition</button>
            <button onClick={() => setActiveGrowthView('velocity')} className={`px-3 py-1.5 text-xs font-600 rounded-md transition-all ${activeGrowthView === 'velocity' ? 'bg-white text-[#0D9488] shadow-sm' : 'text-[#6B7A99]'}`}>Publishing Velocity</button>
          </div>
        </div>
        {activeGrowthView === 'acquisition' ? (
          <ResponsiveContainer width="100%" height={220}>
            <AreaChart data={userAcquisitionData}>
              ...
              <Area type="monotone" dataKey="candidates" stroke="#3b82f6" strokeWidth={2} fill="url(#candGrad)" name="Candidates" />
              <Area type="monotone" dataKey="recruiters" stroke="#8b5cf6" strokeWidth={2} fill="url(#recGrad)" name="Recruiters" />
              <Line type="monotone" dataKey="faculty" stroke="#0d9488" strokeWidth={2} dot={false} name="Faculty" />
            </AreaChart>
          </ResponsiveContainer>
        ) : (
          <ResponsiveContainer width="100%" height={220}>
            <BarChart data={publishingVelocityData}>
              <CartesianGrid strokeDasharray="3 3" stroke="#F0F2F7" />
              <XAxis dataKey="week" tick={{ fontSize: 11, fill: '#9BA8C0' }} axisLine={false} tickLine={false} />
              <YAxis tick={{ fontSize: 11, fill: '#9BA8C0' }} axisLine={false} tickLine={false} />
              <Tooltip />
              <Legend wrapperStyle={{ fontSize: 11 }} />
              <Bar dataKey="courses" fill="#0d9488" name="Courses" radius={[3, 3, 0, 0]} />
              <Bar dataKey="interviews" fill="#8b5cf6" name="Interview Pkgs" radius={[3, 3, 0, 0]} />
              <Bar dataKey="assessments" fill="#f59e0b" name="Assessments" radius={[3, 3, 0, 0]} />
            </BarChart>
          </ResponsiveContainer>
        )}
      </div>

      {/* DAU + Course Completion */}
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-4">
        <div className="bg-white rounded-2xl border border-[#E8ECF4] p-5">
          <h3 className="text-sm font-700 text-[#0D1B3E] mb-1">Daily Active Users</h3>
          <p className="text-xs text-[#6B7A99] mb-4">This week — users & sessions</p>
          <ResponsiveContainer width="100%" height={180}>
            <BarChart data={dauData}>
              <CartesianGrid strokeDasharray="3 3" stroke="#F0F2F7" />
              <XAxis dataKey="day" tick={{ fontSize: 11, fill: '#9BA8C0' }} axisLine={false} tickLine={false} />
              <YAxis tick={{ fontSize: 11, fill: '#9BA8C0' }} axisLine={false} tickLine={false} />
              <Tooltip />
              <Bar dataKey="dau" fill="#3b82f6" name="DAU" radius={[3, 3, 0, 0]} />
              <Bar dataKey="sessions" fill="#e0e7ff" name="Sessions" radius={[3, 3, 0, 0]} />
            </BarChart>
          </ResponsiveContainer>
        </div>

        <div className="bg-white rounded-2xl border border-[#E8ECF4] p-5">
          <h3 className="text-sm font-700 text-[#0D1B3E] mb-1">Course Completion Rate</h3>
          <p className="text-xs text-[#6B7A99] mb-4">By subject — enrolled vs completed</p>
          <div className="space-y-2.5">
            {courseCompletionData.map(c => (
              <div key={c.subject}>
                <div className="flex items-center justify-between mb-1">
                  <span className="text-xs font-500 text-[#0D1B3E]">{c.subject}</span>
                  <div className="flex items-center gap-2">
                    <span className="text-[10px] text-[#9BA8C0]">{c.completed.toLocaleString()}/{c.enrolled.toLocaleString()}</span>
                    <span className="text-xs font-700 text-[#0D9488]">{c.rate}%</span>
                  </div>
                </div>
                <div className="h-2 bg-[#F4F6FA] rounded-full overflow-hidden">
                  <div className="h-full bg-gradient-to-r from-teal-500 to-cyan-400 rounded-full transition-all" style={{ width: `${c.rate}%` }} />
                </div>
              </div>
            ))}
          </div>
      ...
      <div className="bg-white rounded-2xl border border-[#E8ECF4] p-5">
        <div className="mb-4">
          <h3 className="text-sm font-700 text-[#0D1B3E]">Content Performance Heatmap</h3>
          <p className="text-xs text-[#6B7A99]">Active users by day and time slot — darker = more activity</p>
        </div>
          ...
          <div className="min-w-[480px]">
            ...
            <div className="flex items-center gap-1 mb-2">
              ...
              {['6am', '9am', '12pm', '3pm', '6pm', '9pm'].map(t => (
                <div key={t} className="flex-1 text-center text-[10px] font-600 text-[#9BA8C0]">{t}</div>
              ))}
            </div>
            {contentHeatmapData.map(row => (
              <div key={row.day} className="flex items-center gap-1 mb-1">
                <div className="w-10 shrink-0 text-[11px] font-600 text-[#6B7A99]">{row.day}</div>
                {(['6am', '9am', '12pm', '3pm', '6pm', '9pm'] as const).map(slot => {
                  const val = (row as any)[slot] as number;
                  const intensity = Math.min(val / 360, 1);
                  const opacity = 0.1 + intensity * 0.9;
                  return (
                    <div key={slot} className="flex-1 h-8 rounded-md flex items-center justify-center cursor-default transition-transform hover:scale-105"
                      style={{ backgroundColor: `rgba(13, 148, 136, ${opacity})` }}
                      title={`${row.day} ${slot}: ${val} users`}>
                      <span className="text-[9px] font-600" style={{ color: intensity > 0.5 ? 'white' : '#0D9488' }}>{val}</span>
                    </div>
                  );
                })}
              </div>
            ...
            <div className="flex items-center gap-2 mt-3 justify-end">
              <span className="text-[10px] text-[#9BA8C0]">Low</span>
              {[0.1, 0.3, 0.5, 0.7, 0.9].map(o => (
                <div key={o} className="w-5 h-3 rounded-sm" style={{ backgroundColor: `rgba(13, 148, 136, ${o})` }} />
              ))}
              <span className="text-[10px] text-[#9BA8C0]">High</span>
            </div>
          </div>
        </div>
      </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-4">
        {/* Revenue Trend */}
        <div className="lg:col-span-2 bg-white rounded-2xl border border-[#E8ECF4] p-5">
          <div className="flex items-center justify-between mb-4">
            <div>
              <h3 className="text-sm font-700 text-[#0D1B3E]">Revenue & User Growth</h3>
              <p className="text-xs text-[#6B7A99]">Last 6 months</p>
            </div>
            <span className="text-xs bg-green-50 text-green-700 px-2 py-0.5 rounded-full font-600">+18.3% MoM</span>
          </div>
          <ResponsiveContainer width="100%" height={200}>
            <AreaChart data={revenueData}>
              ...
              <Tooltip formatter={(v: any, n: string) => [n === 'revenue' ? `$${v.toLocaleString()}` : v.toLocaleString(), n === 'revenue' ? 'Revenue' : 'Users']} />
        ...
        <div className="bg-white rounded-2xl border border-[#E8ECF4] p-5">
          <h3 className="text-sm font-700 text-[#0D1B3E] mb-1">User Distribution</h3>
          <p className="text-xs text-[#6B7A99] mb-4">By role</p>
          ...
          <div className="space-y-1.5 mt-2">
            {userRoleData.map(d => (
              <div key={d.name} className="flex items-center justify-between text-xs">
                ...
                <span className="font-600 text-[#0D1B3E]">{d.value.toLocaleString()}</span>
              </div>
            ))}
          </div>
        </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-4">
        <div className="bg-white rounded-2xl border border-[#E8ECF4] p-5">
          <h3 className="text-sm font-700 text-[#0D1B3E] mb-4">Recent Activity</h3>
          <div className="space-y-3">
            {recentActivity.map((a, i) => (
              <div key={i} className="flex items-start gap-3">
                <div className="w-7 h-7 rounded-lg bg-[#F0F2F7] flex items-center justify-center shrink-0 text-[#6B7A99]">{a.icon}</div>
                <div className="flex-1 min-w-0">
                  <p className="text-xs font-600 text-[#0D1B3E]">{a.action}</p>
                  <p className="text-[11px] text-[#6B7A99] truncate">{a.entity}</p>
                </div>
                <span className="text-[10px] text-[#9BA8C0] whitespace-nowrap shrink-0">{a.time}</span>
              </div>
            ))}
          </div>
        </div>
        ...
        <div className="bg-white rounded-2xl border border-[#E8ECF4] p-5">
          <h3 className="text-sm font-700 text-[#0D1B3E] mb-4">Quick Actions</h3>
          <div className="grid grid-cols-2 gap-3">
            {[
              { label: 'Add User', icon: <UserPlus size={16} />, color: 'bg-blue-50 text-blue-600' },
              { label: 'Publish Course', icon: <BookOpen size={16} />, color: 'bg-teal-50 text-teal-600' },
              { label: 'Add Interview Pkg', icon: <Mic size={16} />, color: 'bg-violet-50 text-violet-600' },
              { label: 'View CRM Pipeline', icon: <Kanban size={16} />, color: 'bg-amber-50 text-amber-600' },
              { label: 'Manage Institutions', icon: <Building2 size={16} />, color: 'bg-rose-50 text-rose-600' },
              { label: 'Billing Overview', icon: <DollarSign size={16} />, color: 'bg-green-50 text-green-600' },
            ].map(a => (
              <button key={a.label} className={`flex items-center gap-2 p-3 rounded-xl ${a.color} hover:opacity-80 transition-opacity text-left`}>
                {a.icon}
...
function CRMPipelineTab() {
  ...
  const [newDeal, setNewDeal] = useState({ name: '', company: '', contact: '', email: '', value: '', type: 'institution\' as \'institution\' | \'organization', stage: 'prospect' as CRMStage });
...
export default function SuperAdminContent() {
  ...
  return (
    <div className="fade-in">
      ...
      <div className="flex items-start justify-between gap-4 mb-6 flex-wrap">
        <div className="flex items-center gap-4">
          ...
          <div>
            ...
            <p className="text-sm text-[#6B7A99] mt-0.5">CRM · User Management · Content Control · Platform Administration</p>
          </div>
        </div>
        <div className="flex items-center gap-2">
          <span className="flex items-center gap-1.5 text-xs bg-green-50 text-green-700 border border-green-200 px-3 py-1.5 rounded-full font-600">
      ...
      {activeTab === 'institutional-control' && <InstitutionalControlTab />}
      {activeTab === 'organizations' && <OrganizationsTab />}
      {activeTab === 'hiring-analytics' && <HiringAnalyticsTab />}
