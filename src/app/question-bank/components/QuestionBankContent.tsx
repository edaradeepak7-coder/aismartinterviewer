...
import { Search, Plus, Filter, Download, Upload, Code2, Mic, CheckSquare, Edit2, Trash2, Copy, Eye, ChevronDown, X, BookOpen, Building2, RefreshCw, GitBranch, Clock, CheckCircle2, FileText, AlertCircle, CheckCircle, Loader2 } from 'lucide-react';
...
function mapDbQuestion(q: {
  ...
  const raw = (q.difficulty || 'Medium').toLowerCase();
  const difficulty: Difficulty =
    raw === 'easy' || raw === 'hard' || raw === 'medium' ? (raw as Difficulty) : 'medium';
  return {
    id: q.id,
    type: 'interview',
    title: q.text,
    subject: q.category || 'Technical',
    tags: q.technology ? [q.technology] : [],
    difficulty,
    version: 1,
    reuseCount: q.usage_count || 0,
    createdAt: q.created_at?.slice(0, 10) || '',
    updatedAt: q.created_at?.slice(0, 10) || '',
    status: q.is_active === false ? 'archived' : 'active',
  };
}

const FALLBACK_SUBJECTS = ['All Subjects', 'Technical', 'Behavioral', 'Problem Solving', 'Architecture', 'Experience', 'Role Specific'];
...
const DEFAULT_FORM: QuestionFormData = {
  type: 'interview', title: '', subject: 'Technical', company: '', tags: '', difficulty: 'medium',
...
export default function QuestionBankContent() {
  ...
  const loadQuestions = useCallback(async () => {
    ...
    setLoadError(null);
    try {
      const res = await fetch('/api/questions?active=false&limit=200');
      const json = await res.json();
      if (!res.ok) throw new Error(json.error || 'Failed to load questions');
      const rows = Array.isArray(json.data) ? json.data : [];
      setQuestions(rows.map(mapDbQuestion));
    } catch (err: any) {
  ...
  const SUBJECTS = useMemo(() => {
    const fromData = Array.from(new Set(questions.map((q) => q.subject).filter(Boolean))).sort();
    return fromData.length > 0 ? ['All Subjects', ...fromData] : FALLBACK_SUBJECTS;
  }, [questions]);
  ...
  async function saveQuestion() {
    ...
    const difficultyLabel = form.difficulty.charAt(0).toUpperCase() + form.difficulty.slice(1);
    setSaving(true);
    try {
      if (editingQuestion) {
        const res = await fetch(`/api/questions/${editingQuestion.id}`, {
          ...
          body: JSON.stringify({
            ...
            category: form.subject.trim(),
            difficulty: difficultyLabel,
            technology: tags[0] || null,
            is_active: editingQuestion.status !== 'archived',
          }),
        });
        const json = await res.json();
    ...
    try {
      if (bulkAction === 'delete') {
        await Promise.all(ids.map((id) =>
          fetch(`/api/questions/${id}`, { method: 'DELETE', headers: csrfHeaders() })
        ));
        ...
        toast.success(`Activated ${ids.length} questions`);
      }
  ...
  async function confirmCSVImport() {
    if (!csvResult?.questions.length) return;
    try {
      let ok = 0;
      for (const q of csvResult.questions) {
        const res = await fetch('/api/questions', {
          method: 'POST',
          headers: csrfHeaders({ 'Content-Type': 'application/json' }),
          body: JSON.stringify({
            text: q.title,
            category: q.subject || 'Technical',
            difficulty: q.difficulty.charAt(0).toUpperCase() + q.difficulty.slice(1),
            technology: q.tags[0] || null,
          }),
        });
      ...
      toast.success(`Imported ${ok} of ${csvResult.questions.length} questions`);
      setCSVStep('done');
      await loadQuestions();
    } catch (err: any) {
    ...
    <div className="p-4 sm:p-6 space-y-6 max-w-[1400px] mx-auto">
      ...
      <div className="flex flex-col sm:flex-row sm:items-start gap-3 sm:gap-4">
        <div className="flex-1">
          ...
          <p className="text-sm text-muted-foreground mt-0.5">
            Live question repository from Supabase — used by mock interviews
          </p>
          {loadError && (
        ...
        <div className="flex items-center gap-2 flex-wrap">
          <button
            ...
            disabled={loading}
            className="flex items-center gap-1.5 px-3 py-2 border border-border rounded-lg text-sm text-muted-foreground hover:text-foreground hover:bg-muted transition-colors min-h-[40px]"
          >
      ...
      {!loading && questions.length === 0 && !loadError && (
        <div className="text-center py-16 border border-dashed border-border rounded-2xl">
          ...
          <p className="text-sm font-600 text-foreground">No questions yet</p>
          <p className="text-xs text-muted-foreground mt-1 mb-4">Add questions here to power live interviews</p>
          <button onClick={openCreate} className="inline-flex items-center gap-1.5 px-4 py-2 bg-primary text-primary-foreground rounded-lg text-sm font-medium">
            <Plus size={14} /> Add Question
          </button>
        </div>
      )}
        ...
        <div className="fixed inset-0 z-50 flex items-end sm:items-center justify-center bg-black/50 p-0 sm:p-4">
          <div className="bg-card border border-border rounded-t-2xl sm:rounded-2xl w-full sm:max-w-2xl max-h-[90vh] overflow-y-auto shadow-2xl">
            ...
            <div className="flex items-center justify-end gap-3 p-5 sm:p-6 border-t border-border">
              ...
              <button onClick={saveQuestion} disabled={saving} className="px-4 py-2.5 bg-primary text-primary-foreground rounded-lg text-sm font-medium hover:bg-primary/90 transition-colors min-h-[44px] disabled:opacity-50">
