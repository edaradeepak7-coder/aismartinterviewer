...
// ─── Empty plan when no subscription row exists ───────────────────────────────
...
export default function SubscriptionContent() {
  ...
  const [activePlan, setActivePlan] = useState<Plan>(EMPTY_PLAN);
  ...
  useEffect(() => {
    ...
    async function fetchSubscription() {
      ...
      try {
        ...
        if (sub) {
          const total = Number(sub.credits_total) || 0;
          const remaining = Number(sub.credits_remaining ?? total) || 0;
          const plan: Plan = {
            id: sub.id || '',
            name: sub.plan_name || 'Subscription',
            tier: (sub.tier || 'Basic') as Plan['tier'],
            ...
            limits: sub.limits || EMPTY_PLAN.limits,
          };
        ...
        if (!cancelled) {
          ...
          setUsageBreakdown([]);
          setHasSubscription(false);
          setLoadError(err?.message || 'Failed to load subscription');
        }
      } finally {
  ...
  const creditsRemaining = Math.max(0, activePlan.monthlyCredits - creditsUsed);
  const usagePct = activePlan.monthlyCredits > 0
    ? Math.round((creditsUsed / activePlan.monthlyCredits) * 100)
    : 0;
  const selectedPkg = topUpPackages.find((p) => p.id === topUpQty);
  const renewalLabel = activePlan.renewalDate
    ? activePlan.renewalDate.slice(0, 7)
    : '—';
    ...
    <div className="min-h-screen bg-slate-50 p-6 space-y-7">
      ...
      {loadError && (
        <div className="rounded-xl border border-amber-200 bg-amber-50 px-4 py-3 text-sm text-amber-900">
          {loadError}. Showing empty subscription state — no sample plan data.
        </div>
      )}
      ...
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-4">
        <StatCard
          label="Active Plan"
          value={loading ? '—' : (hasSubscription ? activePlan.name : 'None')}
          sub={hasSubscription
            ? `${activePlan.billingCycle === 'annual' ? 'Annual' : 'Monthly'} · renews ${renewalLabel}`
        ...
        <StatCard
          ...
          sub={hasSubscription ? 'From your subscription' : 'No quota yet'}
          icon={<Zap size={18} />}
        ...
        <StatCard
          ...
          sub={activePlan.monthlyCredits > 0 ? `${100 - usagePct}% of monthly quota` : '—'}
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-5">
        ...
        <div className="lg:col-span-2 bg-white rounded-xl border border-slate-100 shadow-sm p-6">
          {!hasSubscription && !loading ? (
            <div className="py-10 text-center">
              <p className="text-sm font-600 text-slate-800">No active subscription</p>
              <p className="text-xs text-slate-500 mt-1">Choose a plan below or open Billing to subscribe.</p>
            </div>
          ) : (
            <>
              <div className="flex items-start justify-between flex-wrap gap-3 mb-5">
                <div>
                  ...
                  <p className="text-sm text-slate-500">
                    {activePlan.price > 0 ? `$${activePlan.price}/mo · ${activePlan.billingCycle}` : '—'}
                  </p>
                </div>
                {hasSubscription && (
              ...
              <CreditMeter used={creditsUsed} total={Math.max(activePlan.monthlyCredits, 1)} />
              {activePlan.features.length > 0 ? (
                ...
                <p className="mt-5 text-xs text-slate-400">No feature list on this subscription record.</p>
              )}
            </>
          )}
        </div>
        ...
        <div className="bg-white rounded-xl border border-slate-100 shadow-sm p-5">
          ...
          {usageBreakdown.length === 0 ? (
            <p className="text-xs text-slate-400 py-8 text-center">No credit usage recorded yet.</p>
          ) : (
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
          {allPlans.map((plan) => {
            const isCurrent = hasSubscription && (
              activePlan.name.toLowerCase().includes(plan.tier.toLowerCase()) ||
              activePlan.tier.toLowerCase() === plan.tier.toLowerCase() ||
              (plan.tier === 'Business' && activePlan.tier === 'Enterprise')
            );
            return (
            <div key={plan.tier} className={`bg-white rounded-xl border-2 ${plan.color} p-5 ${isCurrent ? 'ring-2 ring-amber-300' : ''}`}>
              <div className="flex items-center justify-between mb-2">
                <span className={`text-xs font-700 px-2.5 py-1 rounded-full ${plan.badge}`}>{plan.tier}</span>
                {isCurrent && <span className="text-[10px] font-700 text-amber-600 bg-amber-50 px-2 py-0.5 rounded-full">Current</span>}
              </div>
              ...
              <p className="text-[10px] font-600 text-slate-400 bg-slate-50 rounded-full px-2 py-0.5 inline-block mb-3">{plan.infraReserve} infra reserve</p>
