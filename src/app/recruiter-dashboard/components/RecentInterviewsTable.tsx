...
import { proctoringService, type ProctoringRiskSummary } from '@/lib/services/proctoringService';
import {
...
export default function RecentInterviewsTable() {
  ...
  const [riskByInterview, setRiskByInterview] = useState<Record<string, ProctoringRiskSummary>>({});
  ...
  const fetchInterviews = useCallback(async () => {
    ...
    const ids = data.map((iv) => iv.id).filter(Boolean);
    if (ids.length) {
      const risks = await proctoringService.getRiskByInterviewIds(ids);
      setRiskByInterview(risks);
    } else {
      setRiskByInterview({});
    }
  }, []);
  ...
  // Get proctoring risk from persisted events (fallback: legacy metadata fields)
  const getRiskLevel = (interview: DBInterview): RiskLevel | null => {
    const fromEvents = proctoringService.summarizeForDashboard(riskByInterview[interview.id]);
