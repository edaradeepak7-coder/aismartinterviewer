      ...
      try {
        ...
        if (data) {
          const mapped: ActivityItem[] = data.map((iv: any) => ({
            ...
            role: iv.role || 'Software Engineer',
            type: iv.status,
  ...
  return (
    <div className="bg-card rounded-lg border border-border p-4 h-full flex flex-col fade-in">
      <div className="flex items-center justify-between mb-3">
        ...
        <span className="flex items-center gap-1.5 text-[11px] font-500 text-success">
      ...
      <div className="mt-3 pt-3 border-t border-border">
        <button className="w-full text-center text-[12px] text-primary hover:underline font-500">View all activity</button>
      </div>
    </div>
  );
}