...
function mapDbInstitution(row: any, pendingSeats = 0): Institution {
  return {
    id: row.id,
    name: row.name || 'Institution',
    status: (row.status === 'approved' || row.status === 'pending' || row.status === 'suspended')
      ? row.status
      : 'pending',
    total_seats: row.total_seats || 0,
    used_seats: row.used_seats || 0,
    pending_seats: pendingSeats,
    plan: row.plan || 'Institution Basic',
    approved_at: row.approved_at || undefined,
    auto_renewal: false,
    renewal_date: undefined,
    monthly_cost: (row.used_seats || 0) * SEAT_PRICE_PER_UNIT,
  };
}

function mapDbCandidate(row: any): Candidate {
  return {
    id: row.id,
    ...
    branch: row.branch || '',
    section: row.section || '',
    status: (['active', 'inactive', 'placed', 'interviewing'].includes(row.status) ? row.status : 'inactive') as Candidate['status'],
    score: typeof row.score === 'number' ? row.score : undefined,
    registered_at: row.registered_at || row.created_at || new Date().toISOString(),
    seat_issued: Boolean(row.seat_issued),
  };
}

function mapDbTransaction(row: any): SeatTransaction {
  const amountPaise = typeof row.amount_paise === 'number' ? row.amount_paise : (row.amount || 0) * 100;
  return {
    id: row.id,
    seats_requested: row.seats_requested || 0,
    amount: Math.round(amountPaise / 100),
    payment_method: row.payment_method === 'offline' ? 'offline' : 'online',
    ...
    notes: row.notes || row.offline_reference || undefined,
  };
}
...
function SeatPurchaseModal({ institution, onClose, onSuccess }: {
  ...
  const recordTransaction = async (extra: Record<string, unknown> = {}) => {
    const res = await fetch('/api/institutions/seat-transactions', {
      method: 'POST',
      ...
      body: JSON.stringify({
        ...
        offline_reference: method === 'offline' ? offlineRef.trim() : undefined,
        notes: method === 'offline' ? `Offline ref: ${offlineRef.trim()}` : undefined,
        ...extra,
      }),
    });
    const data = await res.json();
    if (!data.success) throw new Error(data.error || 'Failed to record transaction');
    return data;
  };

  const handlePurchase = async () => {
    if (!institution.id) { setError('No institution linked'); return; }
...
function BulkImportModal({
  ...
  const handleImport = async () => {
    if (importError || !institution.id) return;
    setImporting(true);
    try {
      const res = await fetch(`/api/institutions/${institution.id}/candidates`, {
        method: 'POST',
        headers: csrfHeaders({ 'Content-Type': 'application/json' }),
        body: JSON.stringify({
          candidates: parsedRows.map((c) => ({
            name: c.name,
            email: c.email,
            program: c.program,
            year: c.year,
      ...
      const imported = (data.candidates || []).map(mapDbCandidate);
      onImport(imported);
      setImportResult({ success: imported.length || parsedRows.length, failed: errors.length });
      setStep('result');
    } catch (err: any) {
...
export default function InstitutionAdminContent() {
  ...
  const [institution, setInstitution] = useState<Institution>(EMPTY_INSTITUTION);
  ...
  const loadAll = useCallback(async () => {
    ...
    try {
      ...
      const [candRes, txnRes] = await Promise.all([
        fetch(`/api/institutions/${instRow.id}/candidates`),
        fetch(`/api/institutions/seat-transactions?institution_id=${instRow.id}`),
      ]);
      const candJson = await candRes.json().catch(() => ({}));
      const txnJson = await txnRes.json().catch(() => ({}));

      const txns = Array.isArray(txnJson.transactions)
        ? txnJson.transactions.map(mapDbTransaction)
        : [];
      const pendingSeats = txns
        .filter((t: SeatTransaction) => t.status === 'pending_verification')
        .reduce((s: number, t: SeatTransaction) => s + t.seats_requested, 0);

      setInstitution(mapDbInstitution(instRow, pendingSeats));
      setCandidates(Array.isArray(candJson.candidates) ? candJson.candidates.map(mapDbCandidate) : []);
      setTransactions(txns);
    } catch (err: any) {
      setLoadError(err.message || 'Failed to load institution data');
      setInstitution(EMPTY_INSTITUTION);
      setCandidates([]);
      setTransactions([]);
    } finally {
  ...
  const programs = ['All', ...Array.from(new Set(candidates.map(c => c.program).filter(Boolean)))];
  const years = ['All', ...Array.from(new Set(candidates.map(c => c.year).filter(Boolean)))];
  const courses = ['All', ...Array.from(new Set(candidates.map(c => c.course).filter(Boolean)))];
  const branches = ['All', ...Array.from(new Set(candidates.map(c => c.branch).filter(Boolean)))];
  const sections = ['All', ...Array.from(new Set(candidates.map(c => c.section).filter(Boolean)))];
  ...
  const issueSeat = async (candidateId: string) => {
    ...
    try {
      ...
      const data = await res.json();
      if (!data.success) throw new Error(data.error || 'Failed to issue seat');
      toast.success('Seat issued');
      await loadAll();
    } catch (err: any) {
      toast.error(err.message || 'Failed to issue seat');
    }
  };

  const handleSeatPurchaseSuccess = () => {
    toast.success('Seat purchase recorded');
    loadAll();
  };

  const handleBulkImport = (_importedCandidates: Candidate[]) => {
    loadAll();
  };

  const seatUsagePct = institution.total_seats > 0 ? Math.round((institution.used_seats / institution.total_seats) * 100) : 0;
  const availableSeats = Math.max(0, institution.total_seats - institution.used_seats);
  const unusedSeats = availableSeats;
  ...
  const scoredCandidates = candidates.filter((c) => typeof c.score === 'number');
  const avgScoreLabel = scoredCandidates.length > 0
    ? `${Math.round(scoredCandidates.reduce((s, c) => s + (c.score || 0), 0) / scoredCandidates.length)}%`
    : '—';

  const burnRateTrend = (() => {
    const months: { month: string; seats: number; cost: number; key: string }[] = [];
    const now = new Date();
    for (let i = 5; i >= 0; i--) {
    ...
    transactions
      ...
      .forEach((t) => {
        ...
        if (bucket) {
          bucket.seats += t.seats_requested;
          bucket.cost += t.amount;
        }
      });
    return months.map(({ month, seats, cost }) => ({ month, seats, cost }));
  })();
    ...
    return (
      <AppLayout role="admin">
        <div className="flex items-center justify-center py-24 gap-2 text-[#6B7A99]">
          <Loader2 size={18} className="animate-spin" /> Loading institution…
        </div>
      </AppLayout>
    );
  }
      ...
      <div className="space-y-6 fade-in">
        ...
        {loadError && (
          <div className="bg-amber-50 border border-amber-200 rounded-xl p-4 flex items-start gap-3">
            ...
            <div>
              <p className="text-sm font-700 text-amber-800">Could not load full data</p>
          ...
          <div className="space-y-5">
            <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
              {[
                ...
                { label: 'Avg Score', value: avgScoreLabel, icon: <TrendingUp size={16} />, color: 'bg-amber-50 text-amber-600' },
              ].map(stat => (
            ...
            <div className="bg-white border border-[#E8ECF4] rounded-xl p-5">
              ...
              <div className="space-y-3">
                {candidates.length === 0 ? (
                  <p className="text-sm text-[#6B7A99]">No candidates yet. Import a CSV or add candidates to see branch breakdown.</p>
                ) : Array.from(new Set(candidates.map(c => c.branch).filter(Boolean))).map(branch => {
