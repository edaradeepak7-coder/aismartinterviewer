...
export default function RecruiterStructuredFeedbackContent() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (existingId) {
        await supabase.from('recruiter_structured_feedback').update(payload).eq('id', existingId);
      } else {
        const { data: inserted } = await supabase.from('recruiter_structured_feedback').insert(payload).select().single();
        if (inserted) setExistingId(inserted.id);
      }
      // Update interview recommendation field
      await supabase.from('interviews').update({ recommendation: form.decision }).eq('id', selected.id);
      setSuccess(true);
    } catch {
              ...
              {success && (
                <div className="flex items-start gap-2.5 px-4 py-3 bg-emerald-400/10 border border-emerald-400/20 rounded-lg">
                  ...
                  <p className="text-[13px] text-emerald-400">
                    Feedback {existingId ? 'updated' : 'submitted'} successfully.
                    {form.send_email && form.decision ? ' Email template saved for dispatch.' : ''}
                  </p>
                </div>
              )}
