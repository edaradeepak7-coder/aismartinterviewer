...
import { Code2, Database, Globe, Cpu, Cloud, GitBranch, Search, Zap, Clock, ArrowRight, BookOpen, Target, Filter } from 'lucide-react';
...
/** Subject presets — topics are focus hints, not a live question inventory. */
...
export default function SubjectInterviewsContent() {
  ...
  const [selectedSubject, setSelectedSubject] = useState<SubjectPreset | null>(null);
  const [showCreditModal, setShowCreditModal] = useState(false);
  const { balance, getCost } = useCreditBalance();
  const router = useRouter();

  const availableCount = SUBJECT_PRESETS.filter((s) => s.available).length;
  const comingSoonCount = SUBJECT_PRESETS.filter((s) => !s.available).length;
  const categoryCount = new Set(SUBJECT_PRESETS.map((s) => s.category)).size;
  ...
  const handleStart = (subject: SubjectPreset) => {
  ...
  const handleCreditConfirm = () => {
    ...
    saveInterviewSessionConfig({
      ...
      role: `${selectedSubject.name} Developer`,
      questionTarget,
    });
  ...
  return (
    <div className="fade-in">
      ...
      <div className="flex items-start justify-between gap-4 mb-6">
        <div>
          ...
          <p className="text-sm text-[#6B7A99] mt-0.5">
            Start a real AI interview focused on a technology or subject area
          </p>
        </div>
      ...
      <div className="grid grid-cols-2 sm:grid-cols-4 gap-4 mb-6">
        {[
          { label: 'Available', value: availableCount, icon: <BookOpen size={16} className="text-teal-600" />, color: 'bg-teal-50' },
          { label: 'Coming Soon', value: comingSoonCount, icon: <Clock size={16} className="text-amber-600" />, color: 'bg-amber-50' },
          { label: 'Categories', value: categoryCount, icon: <Filter size={16} className="text-violet-600" />, color: 'bg-violet-50' },
          { label: 'Est. questions', value: `~${questionTarget}`, icon: <Target size={16} className="text-blue-600" />, color: 'bg-blue-50' },
        ].map((s) => (
                ...
                <div className="p-5">
                  ...
                  <div className="flex items-center gap-3 mb-3 text-[11px] text-[#6B7A99]">
                    <span className="flex items-center gap-1">
                      <Target size={10} /> ~{questionTarget} questions
                    </span>
                    <span className="flex items-center gap-1">
