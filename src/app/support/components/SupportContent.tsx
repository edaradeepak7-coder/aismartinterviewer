...
import { TicketIcon, Plus, Search, Clock, CheckCircle2, AlertCircle, XCircle, Loader2, Send, User, Shield, RefreshCw } from 'lucide-react';
...
export default function SupportContent() {
  ...
  const fetchTickets = useCallback(async () => {
    ...
    try {
      if (!user?.id && !isAdmin) {
        setTickets([]);
        setLoadError('Sign in to view your support tickets.');
        return;
      }
  ...
  // eslint-disable-next-line react-hooks/exhaustive-deps -- supabase client is stable enough for this page
  ...
  const fetchComments = useCallback(async (ticketId: string) => {
    ...
    try {
      ...
      if (err) {
        setComments([]);
        setCommentsError(err.message || 'Failed to load replies.');
    ...
    try {
      ...
      if (data) {
        ...
        if (isAdmin && !selectedTicket.resolved_at) {
          const { error: statusErr } = await supabase.from('support_tickets').update({
            status: 'in_progress',
            first_response_at: new Date().toISOString(),
          }).eq('id', selectedTicket.id);
          if (statusErr) {
            toast.error(statusErr.message || 'Reply saved, but status update failed');
          } else {
  ...
  const handleUpdateStatus = async (ticketId: string, newStatus: string) => {
    ...
    const { error: err } = await supabase.from('support_tickets').update(updates).eq('id', ticketId);
    if (err) {
          ...
          <div className="space-y-2 max-h-[600px] overflow-y-auto pr-1">
            {loading ? (
              ...
              <div className="text-center py-12 px-4">
                ...
                <p className="text-sm font-500 text-foreground">Could not load tickets</p>
              ...
              <div className="p-5 space-y-4 max-h-[300px] overflow-y-auto">
                {commentsLoading ? (
                  ...
                  <div className="text-center py-6 px-2">
                    ...
                    <p className="text-sm text-foreground">Could not load replies</p>
                    <p className="text-xs text-muted-foreground mt-1">{commentsError}</p>
                    <button
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50 backdrop-blur-sm p-4">
          <div className="bg-card border border-border rounded-2xl w-full max-w-lg shadow-2xl">
            <div className="flex items-center justify-between p-5 border-b border-border">
              ...
              <button onClick={() => { setShowNewTicket(false); setError(''); }} className="text-muted-foreground hover:text-foreground">
                <XCircle size={20} />
              </button>
            </div>
