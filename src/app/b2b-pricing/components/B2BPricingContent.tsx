...
import { Check, Zap, Crown, Building2, ChevronDown, ChevronUp, Minus, Plus, Shield, Headphones, Globe, AlertCircle, Coins, X, Info, Loader2, BarChart3 } from 'lucide-react';
...
export default function B2BPricingContent() {
  ...
  const [contactForm, setContactForm] = useState({ name: '', company: '', email: '', seats: '', message: '', plan: '' });
  ...
  const trialHref = (plan: B2BPlan) => {
    const params = new URLSearchParams({
      plan: plan.id,
      seats: String(getSeatCount(plan.id)),
      billing,
    });
    return `/recruiter-signup?${params.toString()}`;
  };

  const scrollToContact = (planId?: string, seatHint?: number) => {
    if (planId) {
      setContactForm((p) => ({
        ...p,
        plan: planId,
        seats: seatHint ? String(seatHint) : p.seats || String(getSeatCount(planId)),
      }));
    }
  ...
  const handleContactSubmit = async (e: React.FormEvent) => {
    ...
    setSubmitting(true);
    try {
      const res = await fetch('/api/b2b-sales-inquiry', {
        method: 'POST',
        headers: csrfHeaders({ 'Content-Type': 'application/json' }),
        body: JSON.stringify(contactForm),
      });
      const json = await res.json().catch(() => ({}));
      if (!res.ok) throw new Error(json?.error || 'Could not send inquiry');
            ...
            return (
              ...
              >
                ...
                {plan.id === 'recruiter-business' ? (
                  <button
                    type="button"
                    onClick={() => scrollToContact(plan.id, seatCount)}
          ...
          {formSubmitted ? (
            ...
            <form onSubmit={handleContactSubmit} className="space-y-4">
              ...
              <select
                ...
                className="w-full bg-slate-900/60 border border-slate-700 rounded-xl px-4 py-2.5 text-sm text-white focus:outline-none focus:border-violet-500"
              >
                <option value="">Plan interest (optional)</option>
                <option value="recruiter-starter">Recruiter Starter</option>
                <option value="recruiter-professional">Recruiter Professional</option>
                <option value="recruiter-business">Recruiter Business</option>
              ...
              <button
                type="submit"
                disabled={submitting}
                className="w-full py-3 bg-violet-600 hover:bg-violet-700 disabled:opacity-50 text-white rounded-xl text-sm font-bold transition-colors inline-flex items-center justify-center gap-2"
              >
