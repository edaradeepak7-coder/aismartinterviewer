...
function UnavailablePanel({ title, detail }: { title: string; detail: string }) {
  return (
    <div className="bg-card border border-border rounded-xl flex flex-col items-center justify-center py-16 px-6 text-center">
      <AlertCircle size={36} className="text-muted-foreground/50 mb-3" />
    ...
    <div className="space-y-6">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        <div>
          ...
          <p className="text-sm text-muted-foreground mt-1">Live subscription seats, payment retries, and credit usage</p>
        </div>
      ...
      {loading ? (
        ...
        <>
          ...
          {activeTab === 'forecast' && (
            <UnavailablePanel
              title="MRR forecast not available"
              detail="Projected growth and churn rates are not tracked. Current MRR is shown on Overview from live subscriptions only."
            />
          )}
          ...
          {activeTab === 'cohort' && (
            <UnavailablePanel
              title="Cohort retention not available"
              detail="Month-over-month retention cohorts are not stored yet. Sample retention curves were removed so this view stays empty until real cohort data exists."
            />
          )}
          ...
          {activeTab === 'ltv' && (
            <UnavailablePanel
              title="LTV estimates not available"
              detail="Lifetime value needs measured retention months. Invented retention multipliers were removed; Overview MRR and Members show live pricing."
            />
          )}
