  ...
  return (
    <div className="bg-card border border-border rounded-lg p-4 fade-in">
      <div className="mb-4">
        ...
        <p className="text-[12px] text-muted-foreground mt-0.5">Question bank counts by category</p>
      </div>
        ...
        <div className="flex-1 space-y-2">
          {data.map((entry, index) => (
            <div key={entry.category} className="flex items-center justify-between gap-2">
              ...
              <span className="text-xs font-600 text-foreground tabular-nums shrink-0">{entry.count}</span>
            </div>
          ))}
