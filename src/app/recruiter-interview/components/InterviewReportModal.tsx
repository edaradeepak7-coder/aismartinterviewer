  ...
  "improvements": ["area1", "area2", "area3"]
      ...
      if (res.ok) {
        ...
        if (match) {
          ...
          setReportSummary(parsed.summary || '');
          setStrengths(parsed.strengths || []);
          setImprovements(parsed.improvements || []);
        }
      }
    } catch (err) {
      setReportSummary(`${config.candidateName} completed the interview for ${config.jobTitle}. Overall performance scored ${evaluationScores.overall}/10 across ${qaHistory.length} questions.`);
      setStrengths(['Completed all questions', 'Engaged throughout interview']);
      setImprovements(['Review technical depth', 'Expand on examples']);
    } finally {
