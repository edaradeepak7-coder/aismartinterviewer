...
import { CheckCircle, Star, TrendingUp, MessageSquare, Video, ChevronDown, ChevronUp, Award, ThumbsUp, ThumbsDown, BarChart2, Clock, Target, Zap, ArrowLeft, Play, Pause, Download, AlertCircle, Loader2, RefreshCw, TrendingDown, ShieldCheck, BookOpen, Users, Lightbulb, MessageCircle, Brain, Mic, CheckSquare, XSquare, Sparkles, ListChecks, AlertTriangle } from 'lucide-react';
...
const getLevel = (score: number): CompetencyScore['level'] => {
...
export default function InterviewResultsContent() {
  ...
  const buildResultData = useCallback((interview: any, result: any) => {
    ...
    setResultData({
      ...
      finalScore,
      communicationScore: result.communication_score || interview.communication_score || Math.round(finalScore * 0.9),
      technicalDepthScore: result.technical_depth_score || interview.technical_score || Math.round(finalScore * 1.05),
      recommendation: (result.recommendation || interview.recommendation || 'maybe') as any,
  ...
  const buildResultDataFromInterview = useCallback((interview: any) => {
    ...
    const commScore = interview.communication_score || score;
    const techScore = interview.technical_score || score;
    setResultData({
      candidateName,
      role: interview.role,
      company: interview.company,
      ...
      recommendation: (interview.recommendation || 'maybe') as any,
      competencies: [
        { name: 'Technical Depth', score: techScore, maxScore: 100, level: getLevel(techScore), feedback: 'Based on interview responses.' },
        { name: 'Problem Solving', score: Math.max(0, techScore - 5), maxScore: 100, level: getLevel(Math.max(0, techScore - 5)), feedback: 'Based on interview responses.' },
        { name: 'System Design', score: Math.max(0, techScore - 8), maxScore: 100, level: getLevel(Math.max(0, techScore - 8)), feedback: 'Based on interview responses.' },
        { name: 'Communication', score: commScore, maxScore: 100, level: getLevel(commScore), feedback: 'Based on interview responses.' },
        { name: 'Role Alignment', score: interview.role_alignment_score || score, maxScore: 100, level: getLevel(interview.role_alignment_score || score), feedback: 'Based on interview responses.' },
      ],
      aiFeedback: [],
  ...
  const processAIResponse = async (responseText: string, interview: any) => {
    try {
      ...
      setResultData({
        ...
        communicationScore: parsed.communicationScore || parsed.finalScore,
        technicalDepthScore: parsed.technicalDepthScore || parsed.finalScore,
        recommendation: parsed.recommendation,
        competencies: parsed.competencies,
...
Return ONLY valid JSON, no markdown. Generate answerFeedbacks for each question in the responses. Generate 3-5 improvementTips personalized to this candidate's performance.`;
  ...
  const handleAction = (action: 'offer' | 'decline') => {
    setActionTaken(action === 'offer' ? 'offered' : 'declined');
    setShowConfirm(null);
  };
  ...
  const rec = recommendationConfig[resultData.recommendation] || recommendationConfig['maybe'];
  const avgBenchmark = benchmarks?.avg || 74;
      ...
      <div className="max-w-5xl mx-auto px-4 sm:px-6 py-6 sm:py-8 space-y-6">
                ...
                <div className="bg-slate-900 border border-border rounded-lg overflow-hidden aspect-video relative flex items-center justify-center">
                  ...
                  <div className="relative z-10 text-center">
                    <button
                      ...
                      className="w-14 h-14 rounded-full bg-primary/90 hover:bg-primary flex items-center justify-center transition-all duration-150 active:scale-95 shadow-lg mx-auto mb-2"
                    >
                      {videoPlaying ? <Pause size={22} className="text-white" /> : <Play size={22} className="text-white ml-0.5" />}
                    </button>
                    <p className="text-xs text-slate-400">{videoPlaying ? 'Playing...' : 'Click to replay'}</p>
                  </div>
                </div>
              </div>
              ...
              <div className="space-y-4">
                ...
                <div className="bg-card border border-border rounded-xl p-6">
                  ...
                  <div className="space-y-4">
                    ...
                    {resultData.competencies.map(comp => (
                      <BenchmarkBar key={comp.name} name={comp.name} score={comp.score} benchmark={benchmarks[comp.name] || avgBenchmark} />
                    ))}
                    {/* Communication and Technical */}
                    <BenchmarkBar name="Communication (Overall)" score={resultData.communicationScore} benchmark={benchmarks.communication || avgBenchmark} />
                    <BenchmarkBar name="Technical Depth (Overall)" score={resultData.technicalDepthScore} benchmark={benchmarks.technical || avgBenchmark} />
                  </div>
                </div>
              </div>
            )}
        ...
        <div className="bg-card border border-border rounded-xl p-5 sm:p-6">
          <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
            ...
            {actionTaken ? (
              <div className={`flex items-center gap-2 px-5 py-3 rounded-lg border ${actionTaken === 'offered' ? 'bg-emerald-400/10 border-emerald-400/25 text-emerald-400' : 'bg-red-400/10 border-red-400/25 text-red-400'}`}>
                <CheckCircle size={16} />
                <span className="text-sm font-600">{actionTaken === 'offered' ? 'Offer Extended' : 'Candidate Declined'}</span>
              </div>
            ) : showConfirm ? (
              <div className="flex items-center gap-3 flex-wrap">
                ...
                <button onClick={() => handleAction(showConfirm)} className={`px-4 py-2 rounded-md text-sm font-600 transition-all duration-150 active:scale-95 ${showConfirm === 'offer' ? 'bg-emerald-500 hover:bg-emerald-600 text-white' : 'bg-red-500 hover:bg-red-600 text-white'}`}>
                  Confirm
                </button>
