...
import {
  Eye, EyeOff, Loader2, Mail, Lock, User, Briefcase, ChevronDown, CheckCircle
} from 'lucide-react';
import { triggerBrevoDrip, trackCandidateSignup } from '@/lib/integrations';
...
export default function RegisterPage() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      // 3. If candidate role, also create a candidates row
      if (role === 'candidate') {
        ...
        if (candidateError) {
          console.error('candidates upsert error:', candidateError.message);
        }
      }

      // 4. Send verification email via Resend edge function
      try {
      ...
      // 5. Trigger Brevo drip campaign for new users
      if (role === 'candidate') {
        triggerBrevoDrip('trial_drip', email.trim().toLowerCase(), fullName.trim());
      }

      // 6. Track signup in analytics
      if (authData.user) {
        trackCandidateSignup(authData.user.id, email.trim().toLowerCase(), role);
      }
      ...
      <div className="min-h-screen bg-background flex items-center justify-center px-4">
        <div className="w-full max-w-md text-center">
          ...
          <p className="text-sm text-muted-foreground mb-6">
            Please confirm your email address before signing in to activate your account.
          </p>
          <Link
            href="/login"
    ...
    <div className="min-h-screen bg-background flex items-center justify-center px-4 py-10">
      <div className="w-full max-w-md">
        ...
        <div className="bg-card border border-border rounded-xl p-8 shadow-sm">
          <h1 className="text-lg font-semibold text-foreground mb-6">Register</h1>
