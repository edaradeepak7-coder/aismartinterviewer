...
import { Building2, User, CreditCard, Users, CheckCircle2, ChevronRight, ChevronLeft, Trash2, Mail, Phone, Globe, MapPin, Lock, Eye, EyeOff, Shield, Zap, Star, Crown, Check, AlertCircle, Loader2, UserPlus, Briefcase, GraduationCap, Settings } from 'lucide-react';
...
const STEPS = [
  ...
  { id: 'admin', label: 'Admin Account', shortLabel: 'Admin', icon: <User size={16} /> },
...
function StepAdminAccount({ data, onChange }: { data: AdminAccount; onChange: (d: AdminAccount) => void }) {
  ...
  const strengthColor = ['', 'bg-red-500', 'bg-amber-500', 'bg-yellow-400', 'bg-teal-400'][strength];

  return (
    <div className="space-y-5">
      <div>
        <h2 className="text-xl font-700 text-white mb-1">Admin Account Creation</h2>
        <p className="text-sm text-white/40">This account will have full administrative access to your institution workspace.</p>
      </div>
      <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
        <div>
        ...
        <div>
          <label className="block text-xs font-600 text-white/50 mb-1.5 uppercase tracking-wide">Password *</label>
          <div className="relative">
            ...
            <input
              ...
              className="w-full bg-white/[0.05] border border-white/[0.08] rounded-xl pl-9 pr-10 py-2.5 text-sm text-white placeholder-white/20 focus:outline-none focus:border-teal-500/50 transition-all"
            />
          ...
          {data.password && (
            <div className="mt-2 space-y-1">
              ...
              <p className={`text-[11px] font-600 ${strength >= 3 ? 'text-teal-400' : strength === 2 ? 'text-amber-400' : 'text-red-400'}`}>{strengthLabel}</p>
            </div>
          )}
        </div>
        <div>
          <label className="block text-xs font-600 text-white/50 mb-1.5 uppercase tracking-wide">Confirm Password *</label>
          <div className="relative">
            ...
            <input
              ...
              className={`w-full bg-white/[0.05] border rounded-xl pl-9 pr-10 py-2.5 text-sm text-white placeholder-white/20 focus:outline-none transition-all ${
                data.confirmPassword && data.password !== data.confirmPassword
                  ? 'border-red-500/50 focus:border-red-500/70' :'border-white/[0.08] focus:border-teal-500/50'
              }`}
            />
          ...
          {data.confirmPassword && data.password !== data.confirmPassword && (
            <p className="mt-1 text-[11px] text-red-400 flex items-center gap-1"><AlertCircle size={11} /> Passwords do not match</p>
          )}
        </div>
      </div>
      <div className="flex items-start gap-3 p-3 bg-teal-500/10 border border-teal-500/20 rounded-xl">
        ...
        <p className="text-xs text-white/50">This admin account will receive full access to manage users, billing, and platform settings. You can add more admins later from the RBAC panel.</p>
      </div>
    </div>
  );
}
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
        ...
        <div className="p-4 bg-white/[0.03] border border-white/[0.06] rounded-2xl space-y-2">
          <div className="flex items-center gap-2 mb-3">
            ...
            <p className="text-xs font-700 text-white/50 uppercase tracking-wider">Admin Account</p>
          </div>
        ...
        <div className="p-4 bg-white/[0.03] border border-white/[0.06] rounded-2xl space-y-2">
          <div className="flex items-center gap-2 mb-3">
            ...
            <p className="text-xs font-700 text-white/50 uppercase tracking-wider">Team Invites</p>
          </div>
          {state.team.length > 0 ? (
            <>
  ...
  const canProceed = (): boolean => {
    ...
    if (currentStep === 1) {
      return !!state.admin.fullName.trim() && !!state.admin.email.trim() &&
        !!state.admin.password && state.admin.password === state.admin.confirmPassword &&
    ...
    return (
      <div className="min-h-screen bg-[#060B18] flex items-center justify-center p-6">
        <div className="max-w-md w-full text-center space-y-6">
          ...
          <div>
            <h1 className="text-2xl font-700 text-white mb-2">Workspace Created!</h1>
            <p className="text-white/40 text-sm">
              <span className="text-white font-600">{state.org.name}</span> is now live on Triveda AI Platform.
              {state.team.length > 0 && ` Invitations have been sent to ${state.team.length} team member${state.team.length !== 1 ? 's' : ''}.`}
            </p>
          </div>
          <div className="grid grid-cols-2 gap-3">
            <div className="p-3 bg-white/[0.04] border border-white/[0.06] rounded-xl text-center">
              ...
              <p className="text-xs text-white/40">Seats Active</p>
            </div>
            <div className="p-3 bg-white/[0.04] border border-white/[0.06] rounded-xl text-center">
              <p className="text-lg font-700 text-violet-400">{state.team.length}</p>
              <p className="text-xs text-white/40">Invites Sent</p>
            </div>
          </div>
