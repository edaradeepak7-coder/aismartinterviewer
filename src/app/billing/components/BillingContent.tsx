...
// ─── Plan definitions ─────────────────────────────────────────────────────────
const PLANS = [
  { id: 'free', name: 'Free', price: 0, credits: 30, overage_paise: 0, color: 'text-slate-600', bg: 'bg-slate-50', border: 'border-slate-200' },
  { id: 'starter', name: 'Starter', price: 499, credits: 100, overage_paise: 500, color: 'text-blue-700', bg: 'bg-blue-50', border: 'border-blue-300' },
  { id: 'growth', name: 'Growth', price: 1499, credits: 350, overage_paise: 400, color: 'text-violet-700', bg: 'bg-violet-50', border: 'border-violet-300' },
  { id: 'pro', name: 'Pro', price: 3499, credits: 1000, overage_paise: 300, color: 'text-amber-700', bg: 'bg-amber-50', border: 'border-amber-300' },
];
  ...
  const handleUpgrade = async (plan: typeof PLANS[0]) => {
    ...
    try {
      const res = await fetch('/api/razorpay/order', {
        ...
        body: JSON.stringify({ amount: plan.price, currency: 'INR', receipt: `sub_${plan.id}_${Date.now()}` }),
      });
          ...
          try {
            ...
            if (user) {
              await supabase.from('subscriptions').upsert({
                ...
                credits_used: 0,
                overage_rate_paise: plan.overage_paise,
                current_period_start: new Date().toISOString(),
                current_period_end: new Date(Date.now() + 30 * 24 * 60 * 60 * 1000).toISOString(),
                renewal_date: new Date(Date.now() + 30 * 24 * 60 * 60 * 1000).toISOString(),
                razorpay_order_id: verifyData.orderId,
