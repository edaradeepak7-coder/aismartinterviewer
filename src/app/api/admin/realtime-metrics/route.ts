...
async function getJobQueueMetrics(supabase: Awaited<ReturnType<typeof createClient>>): Promise<JobQueueMetrics> {
  try {
    ...
    if (waitRes.data && waitRes.data.length > 0) {
      ...
      avgWaitMs = Math.round(
        waitRes.data.reduce(
          (sum: number, j: { created_at: string }) => sum + (now - new Date(j.created_at).getTime()),
          0,
        ) / waitRes.data.length,
      );
    }
...
async function getSessionMetrics(supabase: Awaited<ReturnType<typeof createClient>>): Promise<SessionMetrics> {
  try {
