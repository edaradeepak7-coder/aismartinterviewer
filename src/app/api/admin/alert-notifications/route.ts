 ...
 * when a real-time monitor threshold is breached, and logs rows to alert_notifications.
 */
...
async function sendSlackNotification(webhookUrl: string, payload: AlertPayload): Promise<boolean> {
  ...
  const titlePrefix = payload.isTest ? '[TEST] ' : '';
        ...
        blocks: [
          {
            ...
            text: {
              type: 'plain_text',
              text: `${emoji} ${titlePrefix}${payload.severity === 'critical' ? 'CRITICAL' : 'WARNING'}: ${payload.metricLabel}`,
            },
          },
          {
            type: 'section',
...
async function sendEmailNotification(recipients: string[], payload: AlertPayload): Promise<boolean> {
  ...
  const subject = `${payload.isTest ? '[TEST] ' : ''}[${payload.severity.toUpperCase()}] ${payload.metricLabel} threshold breached`;
  const htmlBody = `
    <div style="font-family: sans-serif; max-width: 600px; margin: 0 auto;">
      <div style="background: ${payload.severity === 'critical' ? '#ef4444' : '#f59e0b'}; color: white; padding: 16px 24px; border-radius: 8px 8px 0 0;">
        <h2 style="margin: 0;">${payload.isTest ? '[TEST] ' : ''}${payload.severity === 'critical' ? '🚨 CRITICAL ALERT' : '⚠️ WARNING ALERT'}</h2>
      </div>
...
export async function POST(req: NextRequest) {
  try {
    ...
    const payload: AlertPayload = {
      metric: sanitizeString(String(body.metric || body.metric_key || 'test')).slice(0, 120),
      metricLabel: sanitizeString(String(body.metricLabel || body.threshold_name || body.metric || 'Test Alert')).slice(0, 200),
      currentValue: Number(body.currentValue ?? body.triggered_value ?? body.warn_value ?? 0),
      threshold: Number(body.threshold ?? body.warn_value ?? 0),
      severity: body.severity === 'critical' ? 'critical' : 'warn',
      unit: sanitizeString(String(body.unit || '')).slice(0, 20),
      ...
      emailRecipients: Array.isArray(body.emailRecipients)
        ? body.emailRecipients.map((e: unknown) => String(e))
        : Array.isArray(body.email_recipients)
          ? body.email_recipients.map((e: unknown) => String(e))
          : [],
      slackEnabled: Boolean(body.slackEnabled ?? body.notify_slack),
      emailEnabled: Boolean(body.emailEnabled ?? body.notify_email),
      thresholdId: body.thresholdId || body.threshold_id || undefined,
      isTest: Boolean(body.isTest),
    };

    const results: { slack?: boolean; email?: boolean; logged?: boolean } = {};

    await Promise.all([
      payload.slackEnabled && payload.slackWebhookUrl
        ? sendSlackNotification(payload.slackWebhookUrl, payload).then(async (ok) => {
            results.slack = ok;
            await logNotification({
              threshold_id: payload.thresholdId,
              threshold_name: payload.metricLabel,
              metric_key: payload.metric,
              triggered_value: payload.currentValue,
              severity: payload.severity,
              channel: 'slack',
              recipient: payload.slackWebhookUrl!.slice(0, 80),
              status: ok ? 'sent' : 'failed',
              error_message: ok ? null : 'Slack webhook request failed',
            });
            results.logged = true;
          })
        : Promise.resolve(),
      payload.emailEnabled && payload.emailRecipients?.length
        ? sendEmailNotification(payload.emailRecipients, payload).then(async (ok) => {
            results.email = ok;
            for (const recipient of payload.emailRecipients!) {
              await logNotification({
                ...
                recipient,
                status: ok ? 'sent' : 'failed',
                error_message: ok ? null : 'Email send failed',
              });
            }
            results.logged = true;
          })
    ...
    // Always log a test row when neither channel sent (so UI still gets a notification log entry)
    if (!results.logged && payload.isTest) {
      await logNotification({
        threshold_id: payload.thresholdId,
        threshold_name: payload.metricLabel,
        metric_key: payload.metric,
        triggered_value: payload.currentValue,
        severity: payload.severity,
        channel: payload.slackWebhookUrl ? 'slack' : 'email',
        recipient: payload.slackWebhookUrl?.slice(0, 80) || payload.emailRecipients?.[0] || 'test',
        status: 'sent',
        error_message: null,
      });
      results.logged = true;
