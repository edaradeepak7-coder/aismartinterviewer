...
export default function RecruiterBulkImportContent() {
  ...
  const loadJobRoles = async () => {
    const { data } = await supabase.from('job_postings').select('id, title, department').eq('status', 'active').limit(50);
  ...
  const loadExistingSlots = async () => {
    ...
    setExistingSlots((data || []).map((s: any) => ({ date: s.slot_date, time: s.start_time?.slice(0, 5) })));
  };
  ...
  const handleImport = async () => {
    setImporting(true);
    let success = 0, failed = 0, conflicts = 0, emailsSent = 0;
    ...
    for (let i = 0; i < slots.length; i++) {
      ...
      if (!candidate || candidate.errors.length) { failed++; continue; }
      if (slot.conflict) { conflicts++; }
      try {
        // Upsert candidate
        const { data: cand } = await supabase
          .from('candidates')
          .upsert({ name: candidate.name, email: candidate.email, phone: candidate.phone, role: roleName, skills: candidate.skills?.split(',').map(s => s.trim()) || [] }, { onConflict: 'email' })
          .select('id')
          .single();
        ...
        if (cand) {
          ...
          await supabase.from('recruiter_availability').insert({
            ...
            end_time: (() => {
              ...
              const end = h * 60 + m + slot.duration;
              return `${Math.floor(end / 60).toString().padStart(2, '0')}:${(end % 60).toString().padStart(2, '0')}:00`;
            })(),
            duration_minutes: slot.duration,
            status: 'available',
            notes: `Bulk import — ${roleName}`,
          });
          success++;
          if (sendConfirmation) emailsSent++;
        } else {
          failed++;
        }
      } catch {
    ...
    setResult({ success, failed, conflicts, emailsSent });
    setImporting(false);
    setStep(4);
  };
      ...
      {step === 4 && result && (
        <div className="max-w-lg mx-auto text-center space-y-6 py-8">
          ...
          <div>
            ...
            <p className="text-sm text-muted-foreground">Candidates have been imported and interview slots created</p>
          </div>
          <div className="grid grid-cols-2 gap-3">
            {[
              ...
              { label: 'Conflicts', value: result.conflicts, color: 'text-amber-400', bg: 'bg-amber-400/10 border-amber-400/20' },
              { label: 'Emails Queued', value: result.emailsSent, color: 'text-blue-400', bg: 'bg-blue-400/10 border-blue-400/20' },
            ].map(item => (
          ...
          <div className="flex gap-3 justify-center">
            ...
            <a href="/recruiter-calendar" className="px-5 py-2 text-sm font-600 bg-primary text-white rounded-md hover:bg-primary/90 transition-colors">
              View Calendar →
            </a>
          </div>
        </div>
